...
export default function NecessidadeInsumosTab() {
  ...
  const [dataFim, setDataFim] = useState<Date>(endOfWeek(addDays(new Date(), 7), { weekStartsOn: 1 }));
  ...
  const handleCalcular = async () => {
    ...
    await calcularNecessidadeInsumos(
  ...
  const exportarParaExcel = () => {
    ...
    const dadosExport = necessidadeInsumos.map(insumo => ({
      ...
      'Custo Médio': insumo.custoMedio.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }),
      'Custo Total': insumo.custoTotal.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })
    }));
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Botão Ver Etapas */}
      {necessidadeInsumos.length > 0 && (
        <Collapsible open={mostrarEtapas} onOpenChange={setMostrarEtapas}>
          <CollapsibleTrigger asChild>
            <Button variant="outline" className="w-full">
              {mostrarEtapas ? (
                <><EyeOff className="w-4 h-4 mr-2" /> Ocultar Etapas</>
              ) : (
                <><Eye className="w-4 h-4 mr-2" /> Ver Etapas</>
              )}
            </Button>
          </CollapsibleTrigger>
      ...
      {/* Estado vazio */}
      {!loading && necessidadeInsumos.length === 0 && (
        <Card>
