...
export const ResumoQuantidadeProdutos = ({
  ...
  const calcularQuantidadesSeparadas = async () => {
    const quantidadesSeparadas: {
      [nome: string]: number;
    } = {};

    // Filtrar pedidos que estão separados ou despachados
    const pedidosSeparados = todosPedidos.filter(pedido => pedido.substatus_pedido === 'Separado' || pedido.substatus_pedido === 'Despachado');
  ...
  const [quantidadesTotais, setQuantidadesTotais] = React.useState<{
    [nome: string]: number;
  }>({});
  const [quantidadesSeparadas, setQuantidadesSeparadas] = React.useState<{
    [nome: string]: number;
  }>({});
  ...
  React.useEffect(() => {
    const carregarQuantidades = async () => {
      ...
      setCalculando(true);
      try {
        const [quantidades, quantidadesSep] = await Promise.all([calcularQuantidadesTotais(), calcularQuantidadesSeparadas()]);
  ...
  const produtosSemEstoque = produtosComQuantidade.filter(([nomeProduto, quantidade]) => {
    ...
    const {
      temEstoque
    } = verificarEstoque(nomeProduto, quantidade, quantidadeSeparada);
    return !temEstoque;
  });
  ...
  if (loadingEstoque || calculando) {
    return <Card className="p-4 mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {[1, 2, 3, 4, 5].map(i => <div key={i} className="border rounded-lg p-3 text-center bg-gray-100 animate-pulse">
  ...
  return <Card className="p-4 mb-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-lg font-semibold">Produtos necessários para separação</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        ...
        return <div key={nomeProduto} className={`border rounded-lg p-3 text-center ${temEstoque ? 'bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200' : 'bg-gradient-to-r from-red-100 to-red-200 border-red-400'}`}>
              <div className="font-medium text-sm text-gray-800 mb-1 truncate" title={nomeProduto}>
                {nomeProduto}
              </div>
              <div className={`text-2xl font-bold ${temEstoque ? 'text-blue-700' : 'text-red-800'}`}>
                {quantidade}
              </div>
              <div className="text-xs text-muted-foreground">unidades</div>
              
              <div className={`mt-2 flex items-center justify-center gap-1 text-xs ${temEstoque ? 'text-green-700' : 'text-red-700'}`}>
      ...
      {produtosSemEstoque.length > 0 && <div className="mt-3 p-2 bg-red-100 border border-red-300 rounded-lg">
          <div className="flex items-center gap-2 text-red-800 text-sm font-medium">
            <AlertTriangle className="h-4 w-4" />
            Atenção: Estoque insuficiente
          </div>
          <div className="text-xs text-red-700 mt-1">
            {produtosSemEstoque.length === 1 ? `O produto ${produtosSemEstoque[0][0]} não possui estoque suficiente.` : `${produtosSemEstoque.length} produtos não possuem estoque suficiente.`}
          </div>
        </div>}
