...
export const DebugInfo = ({ tipo, dadosAtivos }: DebugInfoProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const { isLoading } = useExpedicaoStore();
    ...
    if (tipo === "separacao") {
      return {
        ...
        filtros: [
          ...
          "substatus_pedido IN ('Agendado', null)",
      ...
      return {
        ...
        filtros: [
          "status_agendamento = 'Agendado'",
          "substatus_pedido IN ('Separado', 'Despachado')",
          "data_prevista_entrega = hoje"
        ],
        ultimaAtualizacao: agora.toLocaleString(),
        registrosCarregados: dadosAtivos.length,
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div>
              ...
              <div className="bg-muted p-2 rounded text-xs font-mono mt-1 space-y-1">
                {debugData.filtros.map((filtro, index) => (
                  <div key={index}>• {filtro}</div>
                ))}
              </div>
            </div>
