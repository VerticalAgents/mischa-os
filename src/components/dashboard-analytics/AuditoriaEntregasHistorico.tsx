...
interface EntregaAuditoriaHistorico {
  id: string;
  cliente_id: string;
  cliente_nome: string;
  data: Date;
  ...
  quantidadesPorCategoria: { [key: string]: number };
}

interface AuditoriaEntregasHistoricoProps {
  dataInicio: string;
  dataFim: string;
}
...
export default function AuditoriaEntregasHistorico({ dataInicio, dataFim }: AuditoriaEntregasHistoricoProps) {
  const [entregas, setEntregas] = useState<EntregaAuditoriaHistorico[]>([]);
  ...
  const calcularFaturamentoDetalhado = async (entrega: any) => {
    if (entrega.tipo === 'retorno') return { 
      faturamento: 0, 
      precosPraticados: {}, 
      ticketEntrega: 0,
    ...
    try {
      ...
      const quantidadesPorCategoria: { [key: string]: number } = {};
      ...
      if (entrega.itens && entrega.itens.length > 0) {
        ...
        entrega.itens.forEach((item: any) => {
          ...
          if (precoItem && quantidade > 0) {
            faturamentoTotal += quantidade * precoItem;
            quantidadesPorCategoria[categoriaKey] = (quantidadesPorCategoria[categoriaKey] || 0) + quantidade;
          }
        });
      } else {
        ...
        if (cliente?.categoriasHabilitadas && Array.isArray(cliente.categoriasHabilitadas)) {
          // Usar proporções padrão se configuradas
          if (temProporcoesConfiguradas()) {
            try {
              const quantidadesCalculadas = await calcularQuantidadesPorProporcao(entrega.quantidade);
              ...
              quantidadesCalculadas.forEach((item: any) => {
                ...
                if (produto && produto.categoria_id) {
                  const categoriaKey = produto.categoria_id.toString();
                  const precoCategoria = precosPraticados[categoriaKey];
                  
                  if (precoCategoria) {
                    faturamentoTotal += item.quantidade * precoCategoria;
                    quantidadesPorCategoria[categoriaKey] = (quantidadesPorCategoria[categoriaKey] || 0) + item.quantidade;
                  }
                }
              });
            } catch (error) {
            ...
            // Distribuir igualmente entre categorias habilitadas
            const totalCategorias = cliente.categoriasHabilitadas.length;
            const quantidadePorCategoria = Math.floor(entrega.quantidade / totalCategorias);
            ...
            cliente.categoriasHabilitadas.forEach((categoria: any) => {
              ...
              if (precoCategoria) {
                ...
                quantidadesPorCategoria[categoriaKey] = quantidadePorCategoria;
              }
            });
          }
        }
  ...
  const obterNomeCategoria = (categoriaId: string | number) => {
    ...
    return categoria?.nome || 'Categoria não identificada';
  };
      ...
      try {
        ...
        const entregasComDetalhes = await Promise.all(
          entregasFiltradas.map(async (entrega) => {
            const { faturamento, precosPraticados, ticketEntrega, quantidadesPorCategoria } = await calcularFaturamentoDetalhado(entrega);
  ...
  const exportarCSV = () => {
    ...
    link.download = `auditoria_historico_entregas_${dataInicio}_${dataFim}.csv`;
    link.click();
  };
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Detalhamento das entregas com faturamento calculado
              </CardDescription>
            </div>
                        ...
                        <TableCell colSpan={8} className="bg-muted/50">
                          <div className="py-4 space-y-4">
                            {entrega.itens.length > 0 ? (
                                    ...
                                    return (
                                      <div key={index} className="text-sm p-3 bg-background rounded-lg border">
                                        ...
                                        <div className="text-muted-foreground text-xs space-y-1">
                                          <div>Quantidade: <span className="font-medium">{item.quantidade}</span></div>
                              ...
                              <div>
                                <h4 className="font-medium mb-2">Quantidades por Categoria:</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                  {Object.entries(entrega.quantidadesPorCategoria).map(([categoriaId, quantidade]) => {
                                    const precoCategoria = entrega.precosPraticados[categoriaId] || 0;
                                    const subtotal = quantidade * precoCategoria;
                                    
                                    return (
                                      <div key={categoriaId} className="text-sm p-3 bg-background rounded-lg border">
                                        <div className="font-medium text-primary mb-1">
                                          {obterNomeCategoria(categoriaId)}
                                        </div>
                                        <div className="text-muted-foreground text-xs space-y-1">
                                          <div>Quantidade: <span className="font-medium">{quantidade}</span></div>
                                          <div>Preço unitário: <span className="font-medium text-green-600">R$ {precoCategoria.toFixed(2).replace('.', ',')}</span></div>
                                          <div className="pt-1 border-t">
                                            <span className="font-medium text-green-700">Subtotal: R$ {subtotal.toFixed(2).replace('.', ',')}</span>
                                          </div>
                                        </div>
